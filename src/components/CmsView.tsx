...
export default function CmsView({
  ...
  const handleDeleteProduct = (id: string) => {
    if (products.length <= 1) {
      alert("Unable to delete. System requires at least one premium product to maintain the catalog and CRM stability.");
      return;
    }
                      ...
                      <div key={p.id} id={`cms-prod-card-${p.id}`} className="rounded-xl border border-stone-800 bg-stone-900/30 overflow-hidden flex flex-col justify-between font-sans">
                        ...
                        <div className="p-4 flex-1 flex flex-col justify-between">
                          ...
                          <div className="grid grid-cols-3 gap-1 mt-4 border-t border-stone-800 pt-3">
                            ...
                            <button
                              title={products.length <= 1 ? "Unable to delete when product count is one" : "Delete stone"}
                              disabled={products.length <= 1}
                              onClick={() => handleDeleteProduct(p.id)}
                              className={`rounded py-1 flex items-center justify-center text-[10px] ${
                                products.length <= 1
                                  ? 'bg-stone-950 text-stone-600 border border-stone-900 cursor-not-allowed opacity-40'
                                  : 'bg-stone-900 hover:bg-rose-950 border border-stone-800 hover:border-rose-900 text-stone-400 hover:text-rose-400'
                              }`}
                            >
              ...
              <div className="rounded-2xl border border-stone-800 bg-stone-900/20 p-6 md:p-8">
                ...
                <form onSubmit={handleSaveProduct} className="text-xs font-mono space-y-6">
                  ...
                  <div className="grid sm:grid-cols-2 gap-6 p-4 rounded-xl border border-stone-800 bg-stone-900/30">
                    ...
                    <div>
                      <p className="text-[10px] text-amber-500 font-bold uppercase mb-2 flex items-center gap-1.5">
                        <Flame className="h-3.5 w-3.5" /> Granite Types
                      </p>
                      <div className="space-y-1.5 max-h-40 overflow-y-auto p-2 bg-stone-950 rounded border border-stone-850">
              ...
              <div className="lg:col-span-5 space-y-6">
                ...
                <div className="bg-stone-900 border border-stone-850 rounded-xl p-5 shadow-lg space-y-4 font-sans">
                  ...
                  <div className="space-y-3">
                    ...
                    {/* Logo Image Preview */}
                    <div className="rounded-lg bg-stone-950/50 border border-stone-850 p-3.5 flex flex-col items-center justify-center space-y-2">
                      <p className="text-[10px] text-amber-500 font-bold uppercase tracking-wider self-start">Logo Live Preview</p>
                      {logoUrl ? (
                        <div className="h-16 w-full flex items-center justify-center p-2 rounded bg-stone-900 border border-stone-800 overflow-hidden">
                          <img src={logoUrl} alt="Branding Logo Preview" className="max-h-full max-w-full object-contain" referrerPolicy="no-referrer" />
                        </div>
                      ) : (
                        <div className="h-16 w-full flex items-center justify-center bg-stone-900 border border-stone-800 rounded">
                          <p className="text-[11px] text-stone-500 italic">No custom Logo URL provided</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="space-y-6 border-t border-stone-800/60 pt-6">
                    ...
                    <div className="space-y-2">
                      ...
                      <textarea
                        ...
                        className="w-full bg-stone-950 border border-stone-800 text-stone-300 font-sans text-xs rounded-lg p-3 outline-none focus:border-amber-500 font-normal leading-relaxed"
                        placeholder="Define Export Disclaimer..."
                      />
                    </div>
