                          ...
                          onChange={(e) => {
                            ...
                            if (file) {
                              ...
                              r.onload = () => {
                                ...
                                if (typeof rawBase64 === 'string' && onUpdateLogoUrl) {
                                  // For SVG or small images (< 100KB), use raw base64 directly to avoid canvas scaling and preserve transparency
                                  if (file.type === 'image/svg+xml' || file.size < 102400) {
                                    onUpdateLogoUrl(rawBase64);
                                    return;
                                  }

                                  // Resilient canvas downscaling for larger files
                                  const img = new Image();
                                  img.src = rawBase64;
                                  img.onload = () => {
                                    try {
                                      ...
                                      console.error("Canvas scaling error, falling back to raw upload:", err);
                                      onUpdateLogoUrl(rawBase64);
                                    }
                                  };
                                  img.onerror = () => {
