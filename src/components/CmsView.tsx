      ...
      <div className="mx-auto max-w-7xl">
        ...
        {cmsTab === 'masters' && (
          <div className="space-y-6">
            ...
            <div className="md:col-span-1 space-y-1 bg-stone-900/30 p-2.5 rounded-xl border border-stone-850/60 font-mono text-xs">
              <p className="text-[10px] text-stone-500 font-bold uppercase p-2 border-b border-stone-800">Master Entities</p>
              
              <button
                _id="subtab-sizes"
                onClick={() => setMasterSubTab('sizes')}
                className={`w-full text-left rounded px-3 py-2 transition tracking-wider uppercase font-semibold ${
                ...
                <div className="bg-stone-900 border border-stone-850 rounded-xl p-5 shadow-lg space-y-4 font-sans">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1.5">
                      <span className="text-[10px] text-stone-450 font-bold uppercase block">
                        Upload Logo Image File
                      </span>
                      <label className="group relative flex flex-col items-center justify-center border border-dashed border-stone-800 hover:border-amber-500/50 bg-stone-950 hover:bg-stone-950/80 rounded-lg p-4 cursor-pointer transition text-center">
                        <div className="flex flex-col items-center justify-center space-y-1">
                          <Plus className="h-5 w-5 text-stone-500 group-hover:text-amber-500 transition" />
                          <p className="text-[10px] font-bold text-stone-400 group-hover:text-amber-400">Click to upload logo</p>
                          <p className="text-[8px] text-stone-500">Supports PNG, SVG, JPG (Base64 mapping)</p>
                        </div>
                        <input
                          type="file"
                          accept="image/*"
                          ...
                          onChange={(e) => {
                            ...
                            if (file) {
                              ...
                              r.onload = () => {
                                if (typeof r.result === 'string' && onUpdateLogoUrl) {
                                  onUpdateLogoUrl(r.result);
                                }
                              };
                    ...
                    {/* Logo Preview */}
                    {logoUrl && (
                      <div className="pt-2 flex items-center justify-between border-t border-stone-850/50 text-xs">
                        <div className="flex items-center gap-2">
                          <img src={logoUrl} alt="Logo preview" className="h-7 w-auto object-contain bg-stone-950 p-1 rounded border border-stone-800" />
                          <span className="text-[10px] text-stone-400 font-normal">Live preview active</span>
                        </div>
                        <button
                          type="button"
                          onClick={() => onUpdateLogoUrl?.('')}
                          className="text-[9px] font-bold uppercase text-rose-450 hover:text-rose-450 bg-rose-500/5 hover:bg-rose-500/10 px-2 py-0.5 rounded border border-rose-500/10 cursor-pointer"
                        >
                          Clear
                        </button>
                      </div>
