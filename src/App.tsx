...
export default function App() {
  ...
  return (
    <div id="portal-root" className="min-h-screen bg-stone-950 text-stone-200 selection:bg-amber-500 selection:text-stone-950 relative">
      ...
      {/* ADMIN SESSION CONTROL BAR */}
      {isAdminLoggedIn && (view === 'cms' || view === 'crm') && (
        <div id="admin-session-bar" className="bg-amber-500/10 border-b border-amber-500/20 px-6 py-3 text-xs flex items-center justify-between text-stone-300 font-sans">
          <div className="flex items-center gap-2">
            <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />
            <span>Active Exporter Session: <strong className="font-mono text-white">fierystonetraders@gmail.com</strong> (Authenticated via Mailtrap OTP)</span>
          </div>
          <button
            id="btn-admin-logout"
            onClick={() => {
            ...
            className="text-[10px] uppercase font-mono border border-stone-800 hover:border-amber-500/30 bg-stone-900 rounded px-2.5 py-1 text-stone-300 hover:text-amber-550 transition cursor-pointer font-bold"
          >
            Sign Out Session
          </button>
        </div>
